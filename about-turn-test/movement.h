...
void calibrateCompass() {
  //Rotate back and forth to calibrate compass
  // Serial.println("Calibrating compass");
  uint16_t currentDirection = getCompassBearing();
  uint16_t directionToDrive = normalise(currentDirection + 170);
  rotateTo(directionToDrive);
  rotateTo(currentDirection);
  directionToDrive = normalise(currentDirection - 170);
  rotateTo(directionToDrive);
  rotateTo(currentDirection);
  waitUntilCalibrated();
