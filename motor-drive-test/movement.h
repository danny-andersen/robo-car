...
float headingError(float target, float current) {
    ...
    // Wrap into [-TWO_PI, TWO_PI]
    while (e > TWO_PI)  e -= TWO_PI;
    while (e < -TWO_PI) e += TWO_PI;

    // Convert to shortest signed angle [-PI, +PI]
    if (e > PI)  e -= TWO_PI;
    if (e < -PI) e += TWO_PI;

    e = e / TWO_PI;
    return e; //Fraction of a circle
}
...
void drive(Motor_Direction direction, float straightAheadRad, uint8_t speed) {
  ...
  if (direction == FORWARD || direction == BACK) {
    float current = getCompassBearingRads();
    float error = headingError(straightAheadRad, current);
    float correction = error * speed; //Error as a fraction of speed
    ...
    // systemStatus.humidity = leftSpeed;
    // systemStatus.tempC = rightSpeed;
