...
void setup() {
  Monitor.begin();
  Monitor.println("Starting from AppLab...");
...
void handleDirectionRequest() {
  ...
  Bridge.call("getDirectionToDrive").result(direction.directionToDrive);
...
void handleNextObstacle() {
  ...
  // }
  Bridge.notify("obstacleInFront", obstacles[numObstaclesRx].relDirection, obstacles[numObstaclesRx].width, obstacles[numObstaclesRx].avgDistance);
  numObstaclesRx++;
}
